...
document.addEventListener('click', ({ target }) => {
    ...
    element.id == 'btn-sidebar-mobile' ? sidebar.classList.toggle('active') :
        element.id == 'btn-sidebar-close' ? sidebar.classList.remove('active') :
            element.parentElement.className == 'menu-link' ? localStorage.setItem('active-link', $(element.parentElement).attr('href')) :
                element.closest('#logoutBtn') || element.parentElement.id == 'loginLink' ? localStorage.removeItem('active-link') : null;
});
