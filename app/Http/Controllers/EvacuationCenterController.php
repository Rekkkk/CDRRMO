        ...
        if ($request->ajax()) {
            return DataTables::of($evacuationCenterList)
                ...
                ->addColumn('action', function ($row) {
                    return '<a href="javascript:void(0)" data-toggle="tooltip" data-id="' . $row->id . '" data-original-title="Edit" class="btn-edit p-1.5 mr-2 text-sm updateEvacuationCenter">Edit</a>' . '<a href="javascript:void(0)" data-toggle="tooltip" data-id="' . $row->id . '" data-original-title="Remove" class="removeEvacuationCenter p-1.5 mr-2 text-sm">Remove</a>';
                })
                ->rawColumns(['action'])
                ->make(true);
        }
